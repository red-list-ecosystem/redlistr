...
Polygon or any shapefile data may be read in using sf::st_read() and point data in table format can be read in using read.csv() followed by sf::st_as_sf() to convert points into a spatial object. See function documentation in the sf package for more detailed instructions.
...
List of Ecosystem assessments](scaling-up-assessments.html) for
instruction on how to work with multiple ecosystem maps.
...
format](redlistr-raster-vector.html) for instruction on how to read
ecosystem distribution data in other formats.
...
AOO.grid <- getAOO(mangrove.2017, cell_size = 10000, n_jitter = 150)
```
...
plot(AOO.grid, title = "Remove bottom 1% (default)")
