...
We use function `rast` from the `terra` package to read these rasters. This function requires a valid path to find the files with the raster data. In this case both files are included in the `redlistr` package, so we use the `system.file` function to find the path to those files. More information about formats and functions of the `terra` package can be found
...
ecosystems and layers. For example you can also calculate areas for both layers at once by combining them into a raster stack:

```{r Calculate area of multiple rasters}
a <- getArea(c(mangrove.2000, mangrove.2017))
```

See vignette [Scaling up Red List of Ecosystem assessments](scaling-up-assessments.html) for instruction on how to work with multiple ecosystem maps.
...
baseline ([IUCN 2024](https://doi.org/10.2305/CJDF9122)).
...
The `getAreaTrend` can be used to work with multiple maps representing different ecosystem types and time frames. See vignette [Scaling up Red List of Ecosystem assessments](scaling-up-assessments.html) for instruction on how to work with multiple ecosystem maps.
...
functional form of the decline ([IUCN 2024](https://doi.org/10.2305/CJDF9122)).
...
decline ([IUCN 2024](https://doi.org/10.2305/CJDF9122)).
...
refer to the IUCN Red List of Ecosystems guidelines ([IUCN 2024](https://doi.org/10.2305/CJDF9122)).
...
uncertainty if possible. Please see the guidelines ([IUCN 2024](https://doi.org/10.2305/CJDF9122)) for more information.
...
([IUCN 2024](https://doi.org/10.2305/CJDF9122))
...
```{r Make and view EOO, fig.width=7, fig.height=7, fig.alt='Extent of occurrence of 2017 mangrove distribution map'}
...
```{r Creating AOO grid, fig.width=7, fig.height=7, eval = TRUE, fig.alt='Area of occupancy of 2017 mangrove distribution map'}
...
Both the `getEOO` and `getAOO` functions work on polygons too. See vignette [Ecosystem distribution data in raster and vector format](redlistr-raster-vector.html) for instruction on how to read ecosystem distribution data in other formats.
...
```{r Getting number of AOO grids, eval = TRUE, fig.alt='histogram of values generated with the jitter function'}
...
As per the Red List of Ecosystems guidelines ([IUCN 2024](https://doi.org/10.2305/CJDF9122)), the cells containing the smallest areas of
the ecosystem, and collectively adding to less than 1% of the
ecosystem's total area, could be excluded from the AOO calculation if their contribution to risk spreading is negligible. This prevents tiny,
far-flung fragments of an ecosystem from inflating the AOO. 

This option is set per default but can be turned off using the argument, `bottom.1pct.rule = FALSE`, or the percentage changed using the `percent` argument. For assessments
...
```{r One percent grid, fig.width=14, fig.height=5, eval = TRUE, fig.alt='Testing options of AOO metric for 2017 mangrove distribution map'}
