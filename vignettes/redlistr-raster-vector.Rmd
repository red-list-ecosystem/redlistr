...
## 3. Transforming from vector to raster format
...
# polygon example
polygons <- st_read('./inst/extdata/', 'example_polygons')
...
# input polygons 
polygonsT5 <- st_read('./inst/extdata/', 'example_polygons_t5')
area.change <- getAreaChange(polygons, polygonsT5, names_from_x = "time1", names_from_y = "time5")
...
```{r temporal trends with raster stacks, eval=FALSE}
...
#input polygons
#this function is not supported for lists of polygons. 
```
...
```{r Calculating EOO area, eval = TRUE}
...
Both the `getEOO` and `getAOO` functions work on polygons too, just be
sure to specify which column contains ecosystem names. If all polygons
are from one ecosystem, you can leave out this argument.

```{r EOO and AOO with polygons, eval = F}
...
of iterations with the argument `n_jitter` directly in the `getAOO()`
function.
...
percentage changed using the `percent` argument. For assessments
...
```{r One percent grid, fig.width=14, fig.height=5, eval = FALSE}
