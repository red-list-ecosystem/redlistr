...
Ecosystems](http://iucnrle.org/). These criteria assess the change in the extent of an ecosystem over
...
8, and represents mangrove distribution in 2017. Both of these geographic distributions are
...
location on Earth). This can be acheived by simply plotting the map and checking the
coordinates. Otherwise,  check the distribution maps against satellite images, perhaps
by using packages such as `ggmap`, `plotgooglemaps`, `googleVis` etc.
...
the ecosystem distribution. However, before proceeding it is necessary to know the data by 
investigating their resolution (grain size), extent and spatial properties. Specifically,
the coordinate reference system (CRS) of the datasets must be consistent for 
comparing area.
...
Standards. Furthermore, the guidelines suggest using area estimates from more than two time 
points to estimate change, and providing a measure of uncertainty if possible. Please see 
the guidelines ([Bland et al., 2016](https://portals.iucn.org/library/sites/library/files/documents/2016-010.pdf))
...
```{r Creating AOO grid}
AOO.grid.all <- makeAOOGrid(mangrove.2017, grid.size = 10000,
                            min.percent.rule = F)
...
```{r AOO grid plots, fig.width=7, fig.height=7}
plot(AOO.grid.all)
plot(mangrove.2017, add = T, col = "green", legend = FALSE)
...
For our example, due to the small overall size of the ecosystem and the likelihood
of continued connectivity between the remaining patches, the one percent
rule is not recommended, though we might still want to exclude the very small
patches of mangroves in the Southeast. To do so, we can modify the `percent`
parameter, which adjusts the threshold for the AOO grids:
...
n.AOO <- length(AOO.grid.min.percent)
...
AOO until additional shifts no longer produce improved results. To demonstrate
the capabilities of the function, we will be using a 0.1 percent threshold
again.
