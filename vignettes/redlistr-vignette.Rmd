...
author: "Nicholas J. Murray, Calvin K.F. Lee"
date: "`r Sys.Date()`"
...
## 2. Importing data
In this introduction, we will be focusing mainly on the `redlistr` tools aimed at
...
vector (shapefile) format. We will use the excellent `raster`, `sp`, and `rgeos`
packages to import data, and to enable us to begin to use the functions in the
`redlistr` package.
...
Import the data files that you will analyse. In this example, we will use a base
dataset called 'volcano' to represent a very simple ecosystem distribution
dataset. Rasters are used to represent the distribution of an ecosystem, where a
raster value of 1 denotes its presence. You will note that we generate two
rasters. Raster 1 (r1) represents the distribution of an ecosystem at time 1
(t1, 1990) and raster 2 (r2) represents the distribution of the same ecosystem
at time 2 (t2, 2012).

To import your own data, you can use the `raster` function in the `raster`
package, which can handle many different types of georeferenced spatial data. In
some cases, you may be using classified maps that contain many ecosystems, in
which case you can use the huge functionality of the raster package to extract a
single binary raster suitable for ingestion into a redlistr analyses.

```{r Setting up two example rasters and setting their extents}
...
extent(r2) <- ext # set the extent of raster r2, 100m resolution
```

We can plot the data to view the two distributions. Note that at t2 the
ecosystem is smaller than at t1, due to uniform land clearing around the
ecosystem boundaries. We simulated this process for simplicity here.
...
Now we can begin to use `redlistr`, first we load it from github, using the
`devtools` package.
...
in area of the ecosystem over the time. Area is calculated here using the pixel
...
whole ecosystem. The area calculated is provided in square kilometres. We can
also check the resolution of the data (in metres).
...
The results: at time 1 (1990), the ecosystem was `r a.r1` km2, and at time 2,
(2012) the ecosystem was `r a.r2` km2.
...
A change estimate can also be acheived by passing the input raster datasets
directly to the `getAreaChangefromRast` function in `redlistr`. An analogue
function is also provided (`getAreaChangefromShp`) which accepts vector
(shapefile) data as inputs.
...
We can visualise this change using an image differencing function. This function
creates a new raster of all areas that have been lost between t1 and t2. Note
that an accuracy assessment of this change map can
...
instantaneous rate of change ([Puyravaud 2003](http://www.sciencedirect.com/science/article/pii/S0378112702003353)). For further information about the choice of each of these methods to extrapolate
refer to the IUCN Red List of Ecosystems guidelines ([Bland et al 2016](https://portals.iucn.org/library/sites/library/files/documents/2016-010.pdf)).

```{r Using various methods to calculate rate of decline}
